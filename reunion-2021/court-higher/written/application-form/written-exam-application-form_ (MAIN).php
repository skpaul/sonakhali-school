...
// print_r($_POST); die();
    ...
    $fullName = sanitizeString($registration->name, true);
    $fatherName = sanitizeString($registration->fatherName, true);
    $barName = sanitizeString($registration->barName);
    $seniorAdvocateName = sanitizeString($registration->seniorAdvocateName);
    ...
    try {
        ...
        $cinfo->pupilageNo= $form->label("Pupilage No")->httpPost("pupilageNo")->required()->asString(true)->maxLength(20)->validate();
        $cinfo->pupilageOf= $form->label("Pupilage Of")->httpPost("pupilageOf")->required()->asString(true)->maxLength(20)->validate();
        if(!empty($barName)){
            $cinfo->barAssosiationName= $barName;
        } else {
        ...
        if(!empty($barName)){
            $cinfo->seniorAdvocateName= $seniorAdvocateName;
        } else {
        ...
        $cinfo->isEngaged = $form->label("Engaged?")->httpPost("isEngaged")->required()->asString(false)->maxLength(3)->validate();
        ...
        if($cinfo->isEngaged){
            $cinfo->natureOfEngagement= $form->label("Nature Of Engagement")->httpPost("natureOfEngagement")->required()->asString(true)->maxLength(150)->validate();
            $cinfo->placeOfEngagement= $form->label("Place Of Engagement")->httpPost("placeOfEngagement")->required()->asString(true)->maxLength(150)->validate();
        } else {
            $cinfo->natureOfEngagement= "";
            $cinfo->placeOfEngagement= "";
        }

        $cinfo->isDeclaredInsolvent= $form->label("Declared Insolvent")->httpPost("isDeclaredInsolvent")->required()->asBool()->maxLength(1)->default(1)->validate();

        $cinfo->isDismissed= $form->label("Dismissed")->httpPost("isDismissed")->required()->asString(false)->maxLength(3)->validate();
        ...
        if($cinfo->isDismissed){
            $cinfo->dismissalDate= $form->label("Dismissal Date")->httpPost("dismissalDate")->required()->asDate()->validate()->format('Y-m-d');
            $cinfo->dismissalReason= $form->label("Dismissal Reason")->httpPost("dismissalReason")->required()->asString(true)->maxLength(150)->validate();
        } else {
            $cinfo->dismissalDate= null;
            $cinfo->dismissalReason= "";
        }

        $cinfo->isConvicted= $form->label("Convicted")->httpPost("isConvicted")->required()->asBool()->maxLength(3)->default(false)->validate();

        if($cinfo->isConvicted){
            $cinfo->convictionDate= $form->label("Conviction Date")->httpPost("convictionDate")->required()->asDate()->validate()->format('Y-m-d');
            $cinfo->convictionParticulars= $form->label("Conviction Particulars")->httpPost("convictionParticulars")->required()->asString(true)->maxLength(50)->validate();
        }
        else {
            $cinfo->convictionDate = null;
            $cinfo->convictionParticulars = "";
        }
       
        $cinfo->isCancelledByBar= $form->label("Cancelled By Bar")->httpPost("isCancelledByBar")->required()->asBool()->maxLength(3)->default(false)->validate();
        //TODO: must add a valid batch or session.
        $cinfo->session= 2021; //$form->label("Session")->httpPost("session")->required()->asInteger(false)->maxLength(11)->validate();
        // $cinfo->optedBar= $form->label("Opted Bar")->httpPost("optedBar")->required()->asString(true)->maxLength(150)->default(NULL)->validate();
        // $cinfo->status= $form->label("Status")->httpPost("status")->required()->asString(true)->maxLength(50)->default(NULL)->validate();
    ...
    try{
        ...
        #region LL.B
            ...
            $higherEducations->llbCountry= $form->label("LL.B Country")->httpPost("llbCountry")->required()->asString(true)->maxLength(50)->default(NULL)->validate();
        ...
        #region Graduation (Other)
            ...
            if($higherEducations->hasOtherGrad){
                ...
                $higherEducations->gradOtherCountry= $form->label("Graduation (Other) Country")->httpPost("gradOtherCountry")->required()->asString(true)->maxLength(50)->default(NULL)->validate();
        ...
        #region Masters
            ...
            if($higherEducations->hasMasters){
                ...
                $higherEducations->mastersCountry= $form->label("Masters Country")->httpPost("mastersCountry")->required()->asString(true)->maxLength(50)->default(NULL)->validate();
